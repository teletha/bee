...
public class EncodingUtil {
    ...
    /**
     * Parses and returns an encoded version of the given URL string. Wraps the
     * {@link MalformedURLException} and {@link URISyntaxException} in case the passed URL is
     * invalid.
     *
     * @param url Raw/decoded string form of a URL to parse/encode.
     ...
     *
     * @param baseUrl Base URL to use when constructing the final URL, ie:
     *            scheme://authority/initial.path.
     * @param paths Additional path(s) to append at the end of the base path.
     * @return Composed URL (base + paths) already encoded, separating the individual path
     *         components by "/".
     * @since TODO
     */
    public static String encodeURLToString(String baseUrl, String... paths) {
        ...
        String[] parts = paths == null ? new String[0] : paths.length == 1 ? paths[0].split("/") : paths;
